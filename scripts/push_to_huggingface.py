...
This script uploads JSON data to a Hugging Face Dataset repository,
with support for data-diff checking to skip no-op uploads.

Usage:
    python push_to_huggingface.py \
        --json-path artifacts/json/hvac_climate_telemetry.json \
        --dataset-name shahabsalehi/hvac-climate-data \
...
def get_remote_sha(api: HfApi, dataset_name: str, filename: str) -> str | None:
    """
    Get SHA256 hash of the remote file from the dataset's metadata.
    Returns None if file or metadata doesn't exist.
    """
    try:
...
def push_to_huggingface(
    ...
    Args:
        ...
        dataset_name: HF Dataset repo name (e.g., 'shahabsalehi/hvac-climate-data')
        commit_message: Optional commit message
        force: If True, skip data-diff check
        
    Returns:
        True if data was pushed, False if skipped (no changes)
    """
    ...
    if not force:
        ...
        if remote_sha and remote_sha == local_sha:
            ...
            print("No remote SHA found (first push or metadata missing)")
...
def main():
    ...
    parser.add_argument("--json-path", required=True, help="Path to JSON file to upload")
    ...
    parser.add_argument("--force", action="store_true", help="Force push even if unchanged")
